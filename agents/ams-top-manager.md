---
name: ams-top-manager
description: Use only for a valid Claude AMS delegated-manager work order whose hardest bounded subgraph requires the strongest model available to the active AMS root.
...
Specialization: strongest available bounded management. This profile inherits the active root model, resolving to Fable only when the account and organization make Fable available through `best`; otherwise it resolves to Opus. Use that capability only for a subgraph whose ambiguity, consequence, or coordination burden justifies it.
